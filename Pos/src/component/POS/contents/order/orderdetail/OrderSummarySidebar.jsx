      ...
      <div className="sticky top-0 bg-white z-20 border-b border-gray-200">
        <div className="p-6 border-b border-gray-200">
          ...
          <p className="text-[#4A7BA7] text-sm font-medium">
            {order.table} — {itemCount} item{itemCount !== 1 ? 's' : ''}
          </p>
        </div>
            ...
            {menuItems.map((item, index) => (
              ...
              >
                <div className="flex items-start">
                  ...
                  {/* Qty */}
                  <div className="w-12 text-center text-sm text-gray-700 font-medium shrink-0">
                    {item.qty}
                  </div>
                  ...
                  <div className="w-24 text-right shrink-0">
                    <span className="text-sm font-semibold text-gray-900">
                      {item.price > 0 ? `Rs ${(item.price * item.qty).toLocaleString()}` : '—'}
                    </span>
                  </div>
                </div>
              </motion.div>
